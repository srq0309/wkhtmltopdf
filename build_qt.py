...
def build_msvc(config, basedir):
    ...
    build_qt(qtdir, 'nmake', '%s\\qt\\configure.exe %s' % (rootdir, configure_args))
...
def build_mingw64_cross(config, basedir):
    ...
    build_qt(qtdir, 'make -j%d' % CPU_COUNT,
        '%s/qt/configure %s' % (rootdir, configure_args))
...
def chroot_build_linux_generic(config, basedir):
    ...
    configure_args = qt_config('posix',
        ...
        '-L%s/lib'      % libdir,
        '-DOPENSSL_NO_SSL2')
    build_qt(qtdir, 'make -j%d' % CPU_COUNT, '%s/qt/configure %s' % (rootdir, configure_args))
...
def build_posix_local(config, basedir):
    ...
    build_qt(qt, '%s -j%d' % (make, CPU_COUNT),
        '%s/qt/configure %s' % (rootdir, qt_config('posix', '--prefix=%s' % qt)))
...
def build_osx(config, basedir):
    ...
    build_qt(get_dir('qt'), 'make -j%d' % CPU_COUNT, '%s/qt/configure %s' % (rootdir, configure_args))
...
def main():
    basedir = os.path.join(rootdir, './build/_wkqt')
